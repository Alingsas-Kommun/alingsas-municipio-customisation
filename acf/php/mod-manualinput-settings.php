<?php 


if (function_exists('acf_add_local_field_group')) {

    acf_add_local_field_group(array(
    'key' => 'group_672336ea7ed1a',
    'title' => __('Inställningar: Manuell inmatning', 'municipio-customisation'),
    'fields' => array(
        0 => array(
            'key' => 'field_672336eac6189',
            'label' => __('Aktivera sök', 'municipio-customisation'),
            'name' => 'use_search',
            'aria-label' => '',
            'type' => 'true_false',
            'instructions' => '',
            'required' => 0,
            'conditional_logic' => 0,
            'wrapper' => array(
                'width' => '',
                'class' => '',
                'id' => '',
            ),
            'message' => __('Yes', 'municipio-customisation'),
            'default_value' => 0,
            'ui' => 0,
            'ui_on_text' => '',
            'ui_off_text' => '',
        ),
        1 => array(
            'key' => 'field_68c91a16001ab',
            'label' => __('Visa som slider', 'municipio-customisation'),
            'name' => 'show_as_slider',
            'aria-label' => '',
            'type' => 'true_false',
            'instructions' => __('Visar korten i en slider. Kolumninställningen styr hur många slides som syns. Fungerar med Kort, Block, Box, Segment och Nyheter.', 'municipio-customisation'),
            'required' => 0,
            'conditional_logic' => 0,
            'wrapper' => array(
                'width' => '',
                'class' => '',
                'id' => '',
            ),
            'message' => __('Yes', 'municipio-customisation'),
            'default_value' => 0,
            'ui' => 0,
            'ui_on_text' => '',
            'ui_off_text' => '',
        ),
        2 => array(
            'key' => 'field_68c93c4e001ac',
            'label' => __('Centrera allt textinnehåll', 'municipio-customisation'),
            'name' => 'center_content',
            'aria-label' => '',
            'type' => 'true_false',
            'instructions' => __('Centrerar rubrik, brödtext och knapp i korten.', 'municipio-customisation'),
            'required' => 0,
            'conditional_logic' => 0,
            'wrapper' => array(
                'width' => '',
                'class' => '',
                'id' => '',
            ),
            'message' => __('Yes', 'municipio-customisation'),
            'default_value' => 0,
            'ui' => 0,
            'ui_on_text' => '',
            'ui_off_text' => '',
        ),
    ),
    'location' => array(
        0 => array(
            0 => array(
                'param' => 'post_type',
                'operator' => '==',
                'value' => 'mod-manualinput',
            ),
        ),
        1 => array(
            0 => array(
                'param' => 'block',
                'operator' => '==',
                'value' => 'acf/manualinput',
            ),
        ),
    ),
    'menu_order' => 0,
    'position' => 'side',
    'style' => 'default',
    'label_placement' => 'left',
    'instruction_placement' => 'label',
    'hide_on_screen' => '',
    'active' => true,
    'description' => '',
    'show_in_rest' => 0,
    'acfe_display_title' => '',
    'acfe_autosync' => array(
        0 => 'json',
    ),
    'acfe_form' => 0,
    'acfe_meta' => '',
    'acfe_note' => '',
));

}